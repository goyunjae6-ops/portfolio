...
export default function Chatbot() {
  ...
  useEffect(() => {
    ...
    function updateLift() {
      ...
      const progress = Math.min(1, Math.max(0, 1 - distanceFromBottom / EASE_DISTANCE));
      setLiftPx(Math.round(progress * MAX_LIFT));
    }
  ...
  return (
    <div
      className="fixed bottom-6 right-6 z-[9999] sm:bottom-10 sm:right-10"
      style={{ transform: liftPx ? `translateY(-${liftPx}px)` : undefined }}
    >
      <div className="animate-bounce-in flex flex-col items-end gap-3">
        {open && (
          <div
            role="dialog"
            aria-modal="false"
